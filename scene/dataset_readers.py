...
def readColmapSceneInfo(path, images, eval, llffhold=8):
    ...
    reading_dir = "masked" if images == None else images
...
def readCamerasFromTransforms(path, transformsfile, white_background, extension=".png"):
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        for idx, frame in enumerate(frames):
            ...
            # w2c = np.matmul(np.linalg.inv(c2w), obj_mat)
            ...
            cam_intr = {
                    "fl_x" : fov2focal(fovx, image.size[0]),
                    "fl_y" : fov2focal(fovy, image.size[1]),
                    "cx" : image.size[0]/2,
                    "cy" : image.size[1]/2,
                    "w" : image.size[0],
                    "h" : image.size[1]
            ...
            cam_infos.append(CameraInfo(uid=idx, R=R, T=T, FovY=FovY, FovX=FovX, image=image,
                                        image_path=image_path, image_name=image_name, width=image.size[
                                            0],
                                        height=image.size[1], fid=frame_time, cam_intr = cam_intr, mask=mask))
...
def readNerfiesInfo(path, eval):
    ...
    if not os.path.exists(ply_path):
        ...
        max_pose = 0.5
        num_pts = 100_000
        xyz = np.random.random((num_pts, 3)) * max_pose - max_pose/2
        shs = np.random.random((num_pts, 3)) / 255.0
        pcd = BasicPointCloud(points=xyz, colors=SH2RGB(
            shs), normals=np.zeros((num_pts, 3)))
