...
class Camera(nn.Module):
    def __init__(self, colmap_id, R, T, FoVx, FoVy, image, gt_alpha_mask, image_name, uid,
                 trans=np.array([0.0, 0.0, 0.0]), scale=1.0, data_device="cuda", fid=None, depth=None, cam_intr = None):
        ...
        w2c = torch.tensor(getWorld2View2(R, T, trans, scale)).transpose(0, 1)

        self.world_view_transform = w2c.to(self.data_device)
    ...
    def set_intrisic(self, znear, zfar, fx, fy, cx, cy, H, W):
        self.projection_matrix = getProjectionMatrix2(znear, zfar, fx, fy, 
                                                      cx, cy, H, W).transpose(0, 1).to(self.data_device)
        self.full_proj_transform = (
            self.world_view_transform.unsqueeze(0).bmm(self.projection_matrix.unsqueeze(0))).squeeze(0)
        self.camera_center = self.world_view_transform.inverse()[3, :3]

    def reset_intrisic(self):
        self.projection_matrix = getProjectionMatrix2(self.znear, self.zfar, self.cam_intr["fl_x"], self.cam_intr["fl_y"], 
                                                      self.cam_intr["cx"], self.cam_intr["cy"], self.cam_intr["h"], self.cam_intr["w"]).transpose(0, 1).to(self.data_device)
        self.image_width = self.cam_intr["w"]
        self.image_height = self.cam_intr["h"]
        self.full_proj_transform = (
            self.world_view_transform.unsqueeze(0).bmm(self.projection_matrix.unsqueeze(0))).squeeze(0)
        self.camera_center = self.world_view_transform.inverse()[3, :3]
    ...
    def get_extrinsic(self, viewMat):
        full_proj_transform = (
            viewMat.unsqueeze(0).bmm(self.projection_matrix.unsqueeze(0))).squeeze(0)
